...
public class MuleGrapherTask extends MatchingTask
{
    ...
    private void generateGallery()
    {
        ...
        PrintStream p;
        try {
            out = new FileOutputStream(outputdir.getAbsoluteFile() + File.separator + "index.html");
            p = new PrintStream(out);
            p.println("<html> <body>");
            for (Iterator iter = generatedFiles.iterator(); iter.hasNext();) {
                String file = (String)iter.next();
                p.println("<img src=\"" + file + ".gif\" alt=\"" + file + "\" title=\"" + file
                                + "\"" + "\"/>");
                p.println("<br/>");
            }
            p.println("</body></html> ");
    ...
    private void assertFileSets()
    {
        if (filesets == null || filesets.isEmpty())
            throw new BuildException("empty fileset : you must at least provide one valid file");
    }

    private void assertOutputDir()
    {
        if (outputdir == null) throw new BuildException("outputdir is mandatory");
        if (!(outputdir.exists()))
            throw new BuildException("outputdir doesn't exist :" + outputdir.getAbsolutePath());
