...
public abstract class TagProcessor
{
    ...
    public static void appendProperties(Element element, StringBuffer caption)
    {
        ...
        if (properties != null) {
            for (Iterator iterator = properties.getChildren(MuleTag.ELEMENT_PROPERTY).iterator(); iterator
                ...
                caption
                                .append(property.getAttributeValue(MuleTag.ATTRIBUTE_NAME)
                                                + " :"
                                                + lookupPropertyTemplate(property
                                                                .getAttributeValue(MuleTag.ATTRIBUTE_VALUE))
                                                + "\n");
            }
        }
    ...
    public boolean isTwoWay(Element e)
    {
        ...
        return ("true".equalsIgnoreCase(e.getAttributeValue(MuleTag.ATTRIBUTE_SYNCHRONOUS)) || environment
                        .isDefaultTwoWay());
    }
    ...
    protected static String lookupPropertyTemplate(String template)
    {
        ...
        if (value == null && template.startsWith("${")) {
            value = environment.getProperties().getProperty(
                            template.substring(2, template.length() - 1), template);
        }
        else {
            value = template;
        }
