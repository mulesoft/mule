 ...
 * a Mule Server instance. It is responsible for maintaining the UMOs instances
...
public interface UMOModel extends Lifecycle, Initialisable
{
    ...
    /**
     * Sets the model's name. It is poosible to configure more than one model in
     * a config file. The name can then be used to reference the Model use when
     * starting the server
     * 
    ...
    /**
     * The model's name. It is poosible to configure more than one model in a
     ...
     * a component when an event is received for it.
     * 
    ...
    /**
     * This will be used to build entry points on the components registered with
     * the model.
     * 
     * @param entryPointResolver The entryPointResolver to set. This will be
     *            used to build entry points on the components registered with
     *            the model.
     */
    void setEntryPointResolver(UMOEntryPointResolver entryPointResolver);

    /**
     * Registers a <code>UMODescriptor</code> with the
     * <code>MuleManager</code>. The manager will take care of creating the
     * Mule UMO and, it's component and proxies.
     * 
     ...
     * event to events that UMO components registered with the model understand.
     * The <code>UMOLifecycleAdapterFactory</code> is used by the model to
     * instanciate LifecycleAdapters.
     * 
    ...
    /**
     * A convenience method to set a list of components on the model. This
     * method will most likely be used when the model is being constructed from
     * an IoC container
     * 
     * @param descriptors
     * @throws UMOException
     ...
     * starting some Mule UMOs while letting others continue. When a component
     * is stopped all listeners for that component are unregistered.
     * 
