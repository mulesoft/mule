...
/**
 * <code>UMOSession</code> is the context in which a request is executed.  The session
 ...
 * manage events via the <code>UMOEventContext</code> which  is btainable via the
 * <code>UMOManager</code> or by implementing <code>org.mule.umo.lifecycle.Callable</code>.
 *
...
public interface UMOSession extends Serializable
{
   ...
   public UMOMessage sendEvent(UMOMessage message, String endpointName) throws UMOException;
     ...
     * configured for the event
     *
     * @param event the event message payload to send
     * first on the component configuration and then on the mule manager configuration
     * @throws UMOException if the event fails to be processed by the component or
     * the transport for the endpoint
     */
     ...
     */
    public UMOMessage receiveEvent(UMOEndpoint endpoint, long timeout)
            throws UMOException;

    /**
     * Requests a synchronous receive of an event on the component
     *
     * @param endpointName   the endpoint name identifing the endpointUri on ewhich the event will be received
     * @param timeout        time in milliseconds before the request timesout
     * @return The requested event or null if the request times out
     * @throws UMOException if the request operation fails
     */
