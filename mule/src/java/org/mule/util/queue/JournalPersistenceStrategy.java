...
/**
 * 
 * TODO: this implementation is in alpha stage
 *     defragmentation should be handled correctly 
 * 
	...
	public void run() {
		...
		try {
			while (!Thread.interrupted()) {
					...
					try {
						...
						if (overflowLocation != null) {
							...
							if (m != null && m.size() > 0) {
								...
								//System.err.println("Relocating " + m.size() + " records");
								int nbRecord = 0;
								while (m.size() > 0) {
				...
				/*
				synchronized (markLock) {
					//boolean over = overflow;
					boolean over = true;
					boolean modified = true;
					while (!unusedMarks.isEmpty() && modified) {
						modified = false;
						// Check if we could move some records to save disk space 
						if (over) {
							RecordLocation firstUnused = (RecordLocation) unusedMarks.first();
							SortedSet blockingMarks = pendingMarks.headSet(firstUnused);
							if (blockingMarks != null && blockingMarks.size() > 0 && blockingMarks.size() < unusedMarks.size()) {
								try {
									logger.trace("Relocating " + blockingMarks.size() + " records");
									int nbRecord = 0;
									while (!blockingMarks.isEmpty()) {
										RecordLocation loc = (RecordLocation) blockingMarks.first();
										Packet packet = journal.read(loc);
										RecordLocation newLoc = journal.write(packet, false);
										ObjectInputStream ois = new ObjectInputStream(new ByteArrayInputStream(packet.sliceAsBytes()));
										ois.readByte();
						...
						if (!m.isEmpty()) {
							if (logger.isDebugEnabled()) {
								logger.debug("Marking to " + m.last());
							}
							try {
								journal.setMark((RecordLocation) m.last(), false);
								m.clear();
								modified = true;
