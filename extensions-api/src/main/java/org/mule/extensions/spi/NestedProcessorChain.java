...
public class NestedProcessorChain implements NestedProcessor, MuleContextAware
{
    ...
    {
        MuleEvent muleEvent = new DefaultMuleEvent(event.getMessage(), event);
        return chain.process(muleEvent).getMessage().getPayload();
    }

    public Object process(Object payload) throws Exception
    {
        MuleMessage muleMessage = new DefaultMuleMessage(payload, muleContext);
        MuleEvent muleEvent = new DefaultMuleEvent(muleMessage, event);
        return chain.process(muleEvent).getMessage().getPayload();
    }
