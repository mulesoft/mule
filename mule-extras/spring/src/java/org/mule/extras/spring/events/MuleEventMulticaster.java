 ...
 * </code>
 * <p/>
 * Enpoints are specified as a Mule Url which is used to register a listener for the subscription
 * In the pervious version of the MuleEventMulticaster it was possible to specify wildcard
 * endpoints.  This is still possible but you need to tell the multicaster which specific endpoints to listen
 * on and then your subscription listeners can use wildcards. To register the specific endpoints on the
 * Evnet Multicaster you use the <i>subscriptions</i> property.
 * <p/>
 * <code>
...
public class MuleEventMulticaster implements ApplicationEventMulticaster, ApplicationContextAware
{
    ...
    /**
     * Any logical endpointUri mappings to register with mule.  These allow
     * for friendly names to be used in place of urls i.e.
     * email-orders -> smtp://orders:password@restaurant.com
     */
    private Map endpointMappings = null;

    /**
     * A list of endpoints the eventMulticaster will receive events on
     * Note that if this eventMulticaster has a Mule Descriptor associated
     * with it, these endpoints are ignored and the ones on the Mule Descriptor
     * are used.
     * These are here for convenience,  the event multicaster will use these
     * to create a default MuleDescriptor for itself at runtime
     */
     ...
     * If asynchronous is sset to true, an <code>AsynchronousMessageListener</code> is
    ...
    /**
     * Method is used to dispatch events to listeners registered with the EventManager or
     * dispatches events to Mule depending on the type and state of the event received.
     * If the event is not a Mule event it will be dispatched to any listeners registered that
     * are NOT MuleEventListeners.
     * If the event is a Mule event and there is no source event attached to it, it is
     * assumed that the event was dispatched by an object in the context using context.publishEvent()
     * and will be dispatched by Mule.
     * If the event does have za source event attached to it, it is assumed that the event
     * was dispatched by Mule and will be delivered to any listeners subscribed to the event.
     *
    ...
    private boolean isSubscriptionMatch(String endpoint, String[] subscriptions)
    {
        ...
        for (int i = 0; i < subscriptions.length; i++)
        {
            ...
            //Subscriptions can be full Mule Urls or resource specific such as my.queue
            //if it is a MuleEndpointURI we need to extract the Resource specific part
            ...
            if (listener instanceof MuleSubscriptionEventListener)
            {
                ...
                for (int i = 0; i < subscriptions.length; i++)
                {
                    if (subscriptions[i].indexOf("*") == -1 &&
                            MuleEndpointURI.isMuleUri(subscriptions[i]))
                    {
    ...
    {
        if (endpoint.startsWith("glue") || endpoint.startsWith("soap")
                || endpoint.startsWith("axis"))
        {
        ...
        if (!MuleManager.getInstance().isInitialised())
        {
            ...
            if (connectors.size() > 0)
            {
                ...
                for (Iterator iterator = connectors.entrySet().iterator(); iterator.hasNext();)
                {
                    ...
                    if (c.getName() == null) c.setName(entry.getKey().toString());
        ...
        if (!MuleManager.getInstance().isInitialised())
        {
            ...
            if (transformers.size() > 0)
            {
                ...
                for (Iterator iterator = transformers.entrySet().iterator(); iterator.hasNext();)
                {
                    ...
                    if (t.getName() == null) t.setName(entry.getKey().toString());
    ...
    protected UMOFilter createFilter(String pattern)
    {
        try
        {
            if (getSubscriptionFilter() == null) setSubscriptionFilter(WildcardFilter.class);
            UMOFilter filter = (UMOFilter) ClassHelper.instanciateClass(getSubscriptionFilter(), new Object[]{pattern});
            return filter;
        } catch (Exception e)
        {
     ...
     * email-orders -> smtp://orders:password@restaurant.com
     *
     * @return endpointMappings a map of logical names and endpoiut
     *         url strings
     */
     ...
     * email-orders -> smtp://orders:password@restaurant.com
     *
     * @param endpointMappings a map of logical names and endpoiut
     *                         url strings
     */
