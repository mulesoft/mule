...
/**
 * Defines a set of properties required by JavaMail in order to use IMAP with
 * Microsoft Exchange.
 *
 * @since 3.8.0
 */
public class MicrosoftExchangeImapConnector extends ImapConnector
{

    public MicrosoftExchangeImapConnector(MuleContext context)
    {
        super(context);
    }
    ...
    protected void extendPropertiesForSession(Properties global, Properties local, URLName url)
    {
        ...
        addExchangeAuthenticationProperties(ImapConnector.IMAP, local);
    }

    /**
     * Disables the following authentication methods in order to use LOGIN as this is the one that
     * works with MS Exchange through IMAP.
