...
/**
 * A one-shot utility used to extract the ceritificate used by greenmail.
 * See greenmail-truststore in resources.
 * This program does not need to be run for tests.  It is left as documentation. 
 */
public class GreenMailCertificateExtractor
{

    private static final Log logger = LogFactory.getLog(GreenMailCertificateExtractor.class);

    // cut + paste from greenmail source for DummySSLServerSocketFactory
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        // argh!  it just says yes all the time (see DummyTrustManager source!)
        
        // cannot access certificates by a public interface...
        // so cut + paste
        ...
        while (aliases.hasMoreElements())
        {
            ...
            logger.info(alias); // single alias, "icegreen"
            Certificate certificate = ks.getCertificate(alias);
            logger.info(certificate);
            assert(certificate instanceof X509Certificate);
            X509Certificate x509 = (X509Certificate) certificate;
            
            File certFile = FileUtils.newFile("/tmp/greenmail-" + alias);
            OutputStream out = new FileOutputStream(certFile);
            out.write(x509.getEncoded());
            out.close();
        }
    }
}
