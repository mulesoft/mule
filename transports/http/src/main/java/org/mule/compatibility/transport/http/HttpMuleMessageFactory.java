...
public class HttpMuleMessageFactory extends AbstractMuleMessageFactory {
  ...
  Map<String, Serializable> convertHeadersToMap(Header[] headersArray, String uri) throws URISyntaxException {
  ...
  private void putCookieHeaderInMapAsAClient(Map<String, Serializable> headersMap, final Header header, String uri)
    ...
    try {
      final Cookie[] newCookies = CookieHelper.parseCookiesAsAClient(header.getValue(), cookieSpec, new URI(uri));
