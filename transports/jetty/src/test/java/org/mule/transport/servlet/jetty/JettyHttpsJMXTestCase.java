...
public class JettyHttpsJMXTestCase extends HttpFunctionalTestCase
{
    @Override
    protected String getConfigResources()
    {
        return "jetty-https-enable-jmx-test.xml";
    }
    ...
    public void testSend() throws Exception
    {
        ...
        JettyHttpsConnector connector = (JettyHttpsConnector) muleContext.getRegistry().lookupConnector("jettyConnector");
        assertEquals(1, connector.getHttpServer().getConnectors().length);
        assertTrue(connector.getHttpServer().getConnectors()[0].getStatsOn());
    }
