 ...
 * LICENSE.txt file.
 */
package org.mule.transport.cometd.embedded;
...
/**
 * Creates an 'embedded' Cometd server using Jetty and allows Mule to receiver and send events to browsers. The browser will need to use
 * the <pre>mule.js</pre> class to publish and subscribe events.
 */
public class CometdConnector extends CometdServletConnector
{
    public static final String PROTOCOL = "cometd";
...
//            }
    ...
    public MessageReceiver registerListener(Service service, InboundEndpoint endpoint) throws Exception
    {
        ...
        ((CometdMessageReceiver) receiver).setBayeux(holder.getBayeux());
