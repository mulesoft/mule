...
public class VMMessageReceiver extends TransactedPollingMessageReceiver
{
    ...
    public MuleMessage onCall(MuleMessage message) throws MuleException
    {
        ...
        {
            MuleEvent exceptionEvent = e.getEvent();
            if (exceptionEvent == null)
            {
                exceptionEvent = createMuleEvent(message, null);
            }
            return flowConstruct.getExceptionListener().handleException(e,exceptionEvent).getMessage();
        }
        catch (Exception e)
        {
            if (event == null)
