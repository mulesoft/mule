...
public class DefaultMuleEventEndpointUtils
{
    ...
    {
        event.setEndpointFileds(extractCredentials(endpoint), endpoint.getEncoding(), endpoint.getExchangePattern(), endpoint.getName(), endpoint.getEndpointURI().getUri(),
                endpoint.getResponseTimeout(), endpoint.getTransactionConfig().isTransacted());
