...
public class ErrorHandlerOnErrorHasTypeOrWhen implements Validation {
  ...
  public Optional<ValidationResultItem> validate(ComponentAst onErrorModel, ArtifactAst artifact) {
    ...
    if (!isParameterPresent(onErrorModel, WHEN_CHOICE_ES_ATTRIBUTE) &&
        !isParameterPresent(onErrorModel, TYPE_ES_ATTRIBUTE)) {
  ...
  private boolean isParameterPresent(ComponentAst componentAst, String parameterName) {
    ComponentParameterAst parameterAst = componentAst.getParameter(parameterName);
