...
public abstract class AbstractResourceManager {
     ...
     * All subsequent changes in the same
     * thread made to the map are invisible from other threads 
     * until {@link #commitTransaction()} is called.
     * Use {@link #rollbackTransaction()} to discard your changes. 
     * After calling either method there will be
    ...
    public void commitTransaction(AbstractTransactionContext context) throws ResourceManagerException {
    	...
    	if (context.status == Status.STATUS_MARKED_ROLLBACK) {
    		throw new ResourceManagerException("Transaction is marked for rollback");
    	}
        synchronized (context) {
	        if (logger.isDebugEnabled()) {
     ...
     * No more operations will be allowed until a recovery has been
    ...
    protected void assureStarted() throws ResourceManagerSystemException {
        if (operationMode != OPERATION_MODE_STARTED) {
            throw new ResourceManagerSystemException("FileManager not started");
        }
        // do not allow any further writing or commit or rollback when db is corrupt
        if (dirty) {
            throw new ResourceManagerSystemException("Transaction manager is set to dirty, this *may* mean it is corrupt. No modifications are allowed until a recovery run has been performed!");
        }
    }
    ...
    protected void assureReady() throws ResourceManagerSystemException {
        if (operationMode != OPERATION_MODE_STARTED && operationMode != OPERATION_MODE_STOPPING) {
            throw new ResourceManagerSystemException("FileManager not ready");
        }
        // do not allow any further writing or commit or rollback when db is corrupt
        if (dirty) {
            throw new ResourceManagerSystemException("Transaction manager is set to dirty, this *may* mean it is corrupt. No modifications are allowed until a recovery run has been performed!");
