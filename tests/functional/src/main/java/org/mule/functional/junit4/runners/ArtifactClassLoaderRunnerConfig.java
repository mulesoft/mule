 ...
 * <p/>
 * A best practice is to have a base abstract class for your module tests that extends {@link org.mule.functional.junit4.ArtifactFunctionalTestCase}
 * or {@link org.mule.functional.junit4.MuleArtifactFunctionalTestCase} for mule internal tests, and defines if needed anything related to the
 * configuration with this annotation that will be applied to all the tests that are being executed for the same module.
 * <p/>
 * The concept of module of execution is being backed by a JVM where a JVM is created for running the whole test of the module, in case of maven
 * either IDEs.
 *
 * @since 4.0
 */
...
public @interface ArtifactClassLoaderRunnerConfig
{
    ...
    /**
     * In case if a particular test needs to add extra boot packages to append to the ones already defined in the {@code excluded.properties}, it will
     ...
     * <p/>
     * A default list of extra boot packages is always added to the container class loader, it is defined by the {@code excluded.properties} file.
     */
    String extraBootPackages() default "";

    /**
     * The discovery process will look for classes in classpath annotated with {@link Extension}.
     * This is required due to the scan process for annotated classes in the whole classpath takes time if it has to go over
     * the whole set of classes.
     * <p/>
     * If the test doesn't set a base package it will fail to create the {@link ArtifactClassLoaderRunner}.
     * <p/>
     * As best practices modules should define in an abstract base test class this value to be inherited to all the tests for the module.
     *
     ...
     */
    //TODO: MULE-10083 - Improve how ArtifactClassLoaderRunner classifies the classpath for selecting which artifacts are already bundled within the container
