...
public class InjectDefaultObjectSerializerTestCase extends FunctionalTestCase
{
    ...
    {
        TestObjectSerializerAware objectSerializerAware = muleContext.getInjector().inject(new TestObjectSerializerAware());
        assertThat(muleContext.getObjectSerializer(), is(sameInstance(objectSerializerAware.getObjectSerializer())));
        assertThat(objectSerializerAware.getObjectSerializer(), is(sameInstance(muleContext.getRegistry().get(MuleProperties.OBJECT_SERIALIZER))));
    }

    public static class TestObjectSerializerAware
    {
