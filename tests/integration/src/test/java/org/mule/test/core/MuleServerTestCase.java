...
public class MuleServerTestCase extends AbstractMuleTestCase {
  ...
  public void testMuleServerResource() throws Exception {
    muleServer = new TestMuleServer("org/mule/test/spring/config1/test-xml-mule2-config.xml");
  ...
  public void testMuleServerMultipleSpringConfigArgs() throws Exception {
    muleServer =
        new TestMuleServer("-config", "mule-config.xml,org/mule/test/spring/config1/test-xml-mule2-config.xml");
  ...
  public void testMuleServerBuilerArg() throws Exception {
    muleServer = new TestMuleServer("-builder", "org.mule.runtime.config.spring.SpringXmlConfigurationBuilder");
    assertEquals(ClassUtils.getResource("mule-config.xml", MuleServer.class).toString(), muleServer.getConfigurationResources());
  ...
  public void testMuleServerAppConfig() throws Exception {
    muleServer =
        new TestMuleServer("-config", "mule-config.xml", "-appconfig", "org/mule/test/spring/config1/test-app-config.properties");
    muleServer.initialize();
  ...
  private static final class TestMuleServer extends MuleServer {

    private TestMuleServer(String configResources) throws IllegalArgumentException {
      super(configResources);
    }

    private TestMuleServer(String... args) throws IllegalArgumentException {
      super(args);
    }
