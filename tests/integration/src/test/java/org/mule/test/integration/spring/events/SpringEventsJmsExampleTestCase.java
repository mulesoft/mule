...
/**
 * <code>SpringEventsJmsExampleTestCase</code> is a testcase used to test the example config
 * in the docco.  this test is not run when building this module as it relies on Jms, it's
 * used to verify the example config works.
 *
...
public class SpringEventsJmsExampleTestCase extends AbstractMuleTestCase
{
    ...
    public void testReceiveAsWebService() throws Exception
    {
        ...
        Order order= new Order("Sausage and Mash");
        UMOMessage result = client.send("axis:http://localhost:44444/mule/orderManager?method=processOrder", order, null);
