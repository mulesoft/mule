...
public final class ExternalProcess extends ExternalResource {
  ...
  public ExternalProcess(String... processCommand) {
  ...
  protected void before() throws Throwable {
    ...
    LOGGER.trace("Starting esternal process: " + asList(processCommand));
    ...
    daemonFuture = daemonExecutor.submit(() -> {
      try {
        ...
        while (line != null) {
          ...
          LOGGER.warn(line);
          if (line.contains("org.eclipse.jetty.server.ServerConnector: Started ServerConnector")) {
            Thread.sleep(500);
            started.set(true);
            // return;
          }
