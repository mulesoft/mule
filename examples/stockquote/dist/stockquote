...
if [ 1 = $i ]
then
    exec $MULE_BASE/bin/mule -config ./conf/rest-config.xml
elif [ 2 = $i ]
then
    exec $MULE_BASE/bin/mule -config ./conf/soap-config.xml
elif [ 3 = $i ]
then
    exec $MULE_BASE/bin/mule -config ./conf/wsdl-config.xml
fi
