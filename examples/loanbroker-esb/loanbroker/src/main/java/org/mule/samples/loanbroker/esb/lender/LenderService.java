...
public class LenderService
{
    ...
    {
        Bank[] l = getLenderList(request.getCreditProfile(), new Double(request.getCustomerRequest()
            .getLoanAmount()));
    ...
    private String getEndpoint(String endpoint)
    {
        if (endpoint.startsWith("glue") || endpoint.startsWith("axis") || endpoint.startsWith("xfire") || endpoint.startsWith("soap"))
        {
            ...
            if (i > -1)
            {
                endpoint = endpoint.replaceFirst("\\?", "/" + endpoint + "?method=getLoanQuote\\&");
            }
            else
            {
                endpoint += "/" + endpoint + "?method=getLoanQuote";
            }
        }
        return endpoint;
    }
