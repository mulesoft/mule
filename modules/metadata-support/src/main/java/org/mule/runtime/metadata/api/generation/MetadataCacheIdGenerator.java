...
/**
 * Provides a way to generate the {@link MetadataCacheId} of a given representation of a Component. The generated ID will take
...
public interface MetadataCacheIdGenerator<T> {
  ...
  /**
   * Calculates the {@link MetadataCacheId} required to identify the {@link org.mule.runtime.api.metadata.MetadataKey}s associated
   * to the given {@code component}. This method will ignore the values of the configured MetadataKeys as long as they don't
   * affect the resolution of nested keys.
   *
   * @param component the configured component
   * @return a {@link MetadataCacheId} that identifies the MetadataKeys associated to the component with its current
   *         configuration, or {@link Optional#empty} if no valid identifier can be created for the component.
   */
  Optional<MetadataCacheId> getIdForMetadataKeys(T component);

  /**
   * Calculates the {@link MetadataCacheId} required to identify all the Metadata associated to the given {@code component}, and
   * its siblings, based on the referenced global element configuration. This method will ignore the values of the configured
   * MetadataKeys as long as they don't affect the resolution of nested keys.
   *
   * @param component the configured component
   * @return a {@link MetadataCacheId} that identifies the global Metadata associated to the component with its current
   *         configuration, or {@link Optional#empty} if no valid identifier can be created for the component.
   */
