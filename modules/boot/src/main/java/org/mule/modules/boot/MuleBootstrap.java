 ...
 * <p/>
 * MuleBootstrap class is responsible for constructing Mule's classpath from the Mule home
 * folder.
 */
public class MuleBootstrap
{
    ...
    public static void main(String args[]) throws Exception
    {
        ...
        /*
          ...
          The more 'natural' way would be to create a custom URLClassLoader and configure it,
          but then there's a chicken-and-egg problem, as all classes MuleBootstrap depends on
          would have been loaded by a parent classloader, and not ours. There's no straightforward
          way to change this, and is documented in a Sun's classloader guide. The solution
          would've involved overriding the ClassLoader.findClass() method and modifying the semantics
          to be child-first, but that way we are calling for trouble.

          Hacking the primordial classloader is a bit brutal, but works perfectly in case of
          running from the command-line as a standalone app. All Mule embedding options then
          delegate the classpath config to the embedder (a developer embedding Mule in the app),
          thus classloaders are not modified in those scenarios.
        */
        ...
        // get a Method ref from the normal class, but invoke on a proprietary parent object,
