 ...
 * LICENSE.txt file.
 */
package org.mule.module.extension.file.api.stream;
 ...
 * or fully consumed.
 * <p>
 * Because in most implementations the actual reading of the stream
 * requires initialising/maintaining a connection, instances are created
 * through a {@link LazyStreamSupplier}. This allows such connection/resource
 * to be provisioned lazily. This is very useful in cases such as
 * {@link FileSystem#list(String, boolean, MuleMessage, Predicate)}. Being
 * able to only lazily establish the connections, prevents the connector
 * from opening many connections at the same time, at the risk that many of
 * them might end up not being necessary at the same place.
 *
 * @since 4.0
 */
public abstract class AbstractFileInputStream extends AutoCloseInputStream
{
     ...
     * on the {@link #lock}.
     * <p>
     * Because the actual stream is lazily opened, the possibility exists
     * for this method being invoked before the {@link #streamSupplier}
     * is used. In such case, this method will not fail.
     *
