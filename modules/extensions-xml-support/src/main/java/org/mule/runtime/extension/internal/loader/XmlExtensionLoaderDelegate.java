...
final class XmlExtensionLoaderDelegate {
  ...
  public void declare(ExtensionLoadingContext context) {
    ...
    final String configFileName = modulePath.substring(modulePath.lastIndexOf("/") + 1); //TODO MULE-12291: we would, ideally, leaver either the relative path (modulePath) or the URL to the current config file, rather than just the name of the file (which will be useless from a tooling POV)
