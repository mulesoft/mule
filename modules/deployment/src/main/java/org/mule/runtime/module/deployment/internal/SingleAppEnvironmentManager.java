 ...
 */
public class SingleAppEnvironmentManager implements DeploymentService, SingleAppExecutor {

  private static final String MULE_APP_PATH_ENV = getenv("MULE_APP_PATH");

  private static final String MULE_DOMAIN_PATH_ENV = getenv("MULE_DOMAIN_PATH");

  private static final String MULE_APP_PROPERTIES_PATH = getenv("MULE_APP_PROPERTIES_PATH");
  ...
  public void startApp() {
    ...
    try {
      ...
      try {
        properties.load(this.getClass().getResourceAsStream(MULE_APP_PROPERTIES_PATH));
      } catch (Exception e) {
        // Nothing to do.
      }
      ...
      deploymentService.deployDomain(new File(MULE_DOMAIN_PATH_ENV).toURI());
      deploymentService.deploy(new File(MULE_APP_PATH_ENV).toURI(), properties);
      startupListeners.forEach(StartupListener::onAfterStartup);
    } catch (Exception e) {
      LOGGER.error("Exception on starting app", e);
    }
  }
