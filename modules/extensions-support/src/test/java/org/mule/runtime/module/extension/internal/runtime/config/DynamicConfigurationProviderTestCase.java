...
public class DynamicConfigurationProviderTestCase extends AbstractConfigurationProviderTestCase<HeisenbergExtension> {
  ...
  public void before() throws Exception {
    ...
    when(resolverSet.resolve(with(event))).thenReturn(resolverSetResult);
  ...
  public void resolveCached() throws Exception {
    ...
    verify(resolverSet, times(count)).resolve(with(event));
  }
  ...
  public void resolveCachedWithProviderParams() throws Exception {
    ...
    when(providerResolverSet.resolve(with(event))).thenReturn(mock(ResolverSetResult.class));
    ...
    verify(providerResolverSet, times(count)).resolve(with(event));
    verify(resolverSet, times(count)).resolve(with(event));
  }
  ...
  public void resolveProviderParamsDifferentInstance() throws Exception {
    ...
    verify(resolverSet, times(2)).resolve(with(event));
    verify(providerResolverSet, times(1)).resolve(with(event));
    verify(connectionProviderResolver, times(2)).resolve(with(event));
  }
  ...
  private HeisenbergExtension makeAlternateInstance() throws Exception {
    ...
    mockConfigurationInstance(configurationModel, MODULE_CLASS.newInstance());
    when(resolverSet.resolve(with(event))).thenReturn(alternateResult);
  ...
  public void resolveDynamicConfigWithDifferentEvent() throws Exception {
    ...
    when(resolverSet.resolve(with(event))).thenReturn(mock(ResolverSetResult.class));
