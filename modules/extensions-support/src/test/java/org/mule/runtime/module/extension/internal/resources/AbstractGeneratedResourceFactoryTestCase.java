 ...
 * <p>
 * This class is needed because it's not enough to ensure that the resource factory
 * works fine itself, it's also needed to make sure that it is SPI discoverable, otherwise
 * its product will not be available.
 * <p>
 * Implementations of this class are to implement the {@link #getResourceFactoryTypes()}
 * and return the types that are to be tested, and the {@link #spiDiscovery()} test
 * will verify that all of them can be found through a {@link ServiceLoader}
 *
 * @since 4.0
 */
