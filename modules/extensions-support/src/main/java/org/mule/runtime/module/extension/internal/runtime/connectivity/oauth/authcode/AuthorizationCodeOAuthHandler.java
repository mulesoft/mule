...
public class AuthorizationCodeOAuthHandler extends OAuthHandler<AuthorizationCodeOAuthDancer> {
  ...
  private BiConsumer<AuthorizationCodeDanceCallbackContext, ResourceOwnerOAuthContext> afterCallback(
    ...
    return (callbackContext, oauthContext) -> {
      ...
      event = CoreEvent.builder(event).message(Message.builder(this.getClass().getClassLoader()).value(state).build()).build();
  ...
  private CoreEvent createEvent(Object payload, OAuthConfig config, Flow flow) {
    return CoreEvent.builder(create(flow, from(config.getOwnerConfigName())))
        .message(Message.builder(this.getClass().getClassLoader()).value(payload).build()).build();
  }
