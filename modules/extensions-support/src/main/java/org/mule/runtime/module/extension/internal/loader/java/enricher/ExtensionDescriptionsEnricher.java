...
public final class ExtensionDescriptionsEnricher implements DeclarationEnricher {
  ...
  public void enrich(ExtensionLoadingContext loadingContext) {
    ...
    try (InputStream resource = classLoader.getResourceAsStream("META-INF/" + SERIALIZER.getFileName(name))) {
      if (resource != null) {
        XmlExtensionDocumentationApi documenter = withContextClassLoader(
   ...
   */
  private void document(ExtensionDeclaration declaration, XmlExtensionDocumentationApi documentation) {
    ...
    new DeclarationWalker() {
      ...
      private void document(ParameterizedDeclaration<?> declaration, List<XmlExtensionElementDocumentationApi> elements) {
