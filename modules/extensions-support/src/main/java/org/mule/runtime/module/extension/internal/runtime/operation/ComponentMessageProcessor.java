...
public abstract class ComponentMessageProcessor<T extends ComponentModel> extends ExtensionComponent<T>
  ...
  private ProfilingDataProducer<ComponentExecutionProfilingEventContext> getThreadReleaseDataProducer(ProfilingService profilingService) {
    if (profilingService == null) {
      return null;
    } else {
      return profilingService.getProfilingDataProducer(COMPONENT_THREAD_RELEASE);
    }
  }
