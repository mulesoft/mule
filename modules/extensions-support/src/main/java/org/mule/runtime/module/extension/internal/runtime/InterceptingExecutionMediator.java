 ...
 * <p>
 * It works by wrapping another mediator which is expected to return
 * an {@link InterceptingCallback} ({@link IllegalStateException} will
 * be thrown otherwise). Once the callback has been obtained, it is
 * placed as an {@link OperationContextAdapter} variable with key
 * {@link ExtensionProperties#INTERCEPTING_CALLBACK_PARAM} and
 * returns the value of {@link InterceptingCallback#getResult()}.
 * <p>
 * This class is not responsible from actually invoking the intercepted
 * chain nor invoking additional methods on the callback.
 *
 * @since 4.0
 */
    ...
    public Object execute(OperationExecutor executor, OperationContextAdapter context) throws Throwable
    {
        ...
        if (!(resultValue instanceof InterceptingCallback))
        {
            throw new IllegalStateException(format("operation '%s' was expected to return a '%s' but a '%s' was found instead",
                                                   context.getOperationModel().getName(), InterceptingCallback.class.getSimpleName(), resultValue));
        }
