...
public class InterceptingOperationMessageProcessor extends OperationMessageProcessor implements InterceptingMessageProcessor, MessageProcessorContainer
{
    ...
    public InterceptingOperationMessageProcessor(RuntimeExtensionModel extensionModel,
    ...
    @Override
    protected MuleEvent doProcess(org.mule.runtime.core.api.MuleEvent event, OperationContextAdapter operationContext) throws MuleException
    {
    ...
    private MuleException onException(InterceptingCallback<?> interceptingCallback, MuleEvent event, OperationContextAdapter operationContext, Exception exception)
    {
        ...
        catch (Exception e)
        {
            if (LOGGER.isDebugEnabled())
            {
                LOGGER.debug(format("Intercepting operation '%s' got an error while processing exception on callback. Original exception will be thrown back to the flow"
                        , operationContext.getOperationModel().getName()), e);
            }
        }
    ...
    private void onSuccess(OperationContextAdapter operationContext, MuleEvent resultEvent, InterceptingCallback<?> interceptingCallback) throws MessagingException
    {
    ...
    private void onComplete(InterceptingCallback<?> interceptingCallback, MuleEvent event, OperationContextAdapter operationContext) throws MuleException
    {
        ...
        {
            throw new MessagingException(createStaticMessage(format("Intercepting operation '%s' failed to notify completion", operationContext.getOperationModel().getName())),
                                         event, e, this);
        }
    }
    ...
    private InterceptingCallback<?> getInterceptorCallback(OperationContextAdapter operationContext)
    {
        ...
        {
            throw new IllegalStateException("Could not find callback for intercepting operation " + operationContext.getOperationModel().getName());
        }
    ...
    private MuleEvent processNext(MuleEvent interceptedEvent, OperationContextAdapter operationContext) throws MuleException
    {
        ...
        else
        {
            ...
            if (LOGGER.isDebugEnabled())
            {
                LOGGER.debug(format("Intercepting operation '%s' executed intercepted chain and got the following event back: ", operationContext.getOperationModel().getName(), resultEvent));
            }
