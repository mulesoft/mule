...
/**
 * Implementation of {@link RetryRequest} which has the ability to link every
 * requests to a {@link Interceptor} and keep a history of interceptors which already
 * performed a request.
 * <p/>
 * This is useful to prevent badly written {@link Interceptor interceptors} from
 * generating an endless loop by requesting retries over and over again. If an interceptor
 * already in the {@link #history} sends the request for a second time an {@link IllegalStateException} is thrown.
 *
 * @since 4.0
 */
final class InterceptorsRetryRequest implements RetryRequest
{
    ...
    /**
     * Creates a new instance, copying the history from the {@code previous}
     * request.
     * <p/>
