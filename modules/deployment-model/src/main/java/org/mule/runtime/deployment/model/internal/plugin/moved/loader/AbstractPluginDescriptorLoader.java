...
import org.mule.runtime.deployment.model.internal.plugin.moved.DefaultDescriptor;
import org.mule.runtime.deployment.model.internal.plugin.moved.DefaultPluginDescriptor;
...
public abstract class AbstractPluginDescriptorLoader {
  ...
  private final static File PLUGIN_JSON_DESCRIPTOR_FILE = new File(META_INF_FOLDER, "mule-plugin.json");
  ...
  public PluginDescriptor load(File pluginLocation) throws MalformedPluginException {
    ...
    if (!jsonDescriptorInputStream.isPresent()) {
      throw new MalformedPluginException(format("There is no %s descriptor available for the current plugin %s",
