 ...
 * LICENSE.txt file.
 */
package org.mule.runtime.module.artifact.activation.internal.service.manager;
...
import org.mule.runtime.module.artifact.activation.internal.service.discoverer.LazyServiceAssembly;
...
public class LazyServiceProxy implements InvocationHandler {
  ...
  private synchronized void doStop() {
    withServiceClassLoader(() -> {
      try {
        ...
        LOGGER.warn(String.format("Service '%s' was not stopped properly: %s", assembly.getName(), e.getMessage()), e);
      }
      ...
      if (assembly.getClassLoader() instanceof DisposableClassLoader) {
        try {
          ...
          LOGGER
              .warn(String.format("Service '%s' class loader was not stopped properly: %s", assembly.getName(), e.getMessage()),
                    e);
        }
  ...
  private void withServiceClassLoader(CheckedRunnable task) {
    ClassUtils.withContextClassLoader(assembly.getClassLoader(), task);
  }

  private <T> T withServiceClassLoader(Callable<T> callable) {
    return ClassUtils.withContextClassLoader(assembly.getClassLoader(), callable);
  }
