...
/**
 * A URL Connection to a Mule Plugin file or an entry in a Mule Plugin file.
 *
...
public class MulePluginURLConnection extends URLConnection {
  ...
  /**
   * As this class is meant to help classloaders look for resources within zips, the "classes" is a particular scenario
   ...
   *
   * @param file to split by {@link #SEPARATOR}
  ...
  private List<String> getFiles(final String file) {
    ...
    //TODO MULE-10873 this if should be different, it should be if files.length == 2, it means the first element of the array is the zip content and the second one is actually a resource to access it through ZipFile. No need to ask for "classes" as we dont actually care(maybe a validation to guarantee the first element is a"zip"..)
