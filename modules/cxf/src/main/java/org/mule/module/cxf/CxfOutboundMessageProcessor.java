...
public class CxfOutboundMessageProcessor extends AbstractInterceptingMessageProcessor implements CloneableMessageProcessor, NonBlockingSupported
{
    ...
    protected MuleEvent doSendWithProxy(MuleEvent event) throws Exception
    {
        ...
        // CXF blocks until the until the response value is aviailable and then proceeds with response processing on the
        // waiting thread.  When non-blocking is enabled the message owner will be a different thread and so this needs
        // reseting here.
