...
public class ProxyValidationComparisonTestCase extends FunctionalTestCase
{
    ...
    private static final String SAMPLE_REQUEST = "<soap:Envelope xmlns:soap=\"http://schemas.xmlsoap.org/soap/envelope/\">"
                                                + "<soap:Body>"
    ...
    {
        return muleContext.getClient().send(String.format("http://localhost:%s/services/%s", httpPort.getNumber(), path), getTestMuleMessage(SAMPLE_REQUEST), HTTP_REQUEST_OPTIONS);
    }
