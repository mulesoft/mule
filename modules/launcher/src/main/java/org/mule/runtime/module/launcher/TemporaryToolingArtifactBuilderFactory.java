...
/**
 * Default implementation of {@code ToolingArtifactBuilderFactory} that allows to create a builder
 * use to create a temporary artifact for tooling services.
 *
 * @since 4.0
 */
        ...
        return new ToolingArtifactBuilder()
        {
            ...
            private MuleContextListener createMuleContextListener()
            {
                return new MuleContextListener()
                {
                    ...
                    public void onCreation(MuleContext context)
                    {
                        ...
                        {
                            context.getCustomizationService().registerCustomServiceClass(connectivityTestingStrategyType.getName(), connectivityTestingStrategyType);
                        }
                    }
            ...
            public ToolingArtifact build()
            {
                try
                {
                    ...
                    artifactPluginDescriptors.addAll(this.artifactPluginFiles.stream()
                            .map(file -> {
                                try
                                {
                    ...
                    temporaryContextClassLoader = muleArtifactResourcesRegistry.getTemporaryArtifactClassLoaderBuilderFactory().createArtifactClassLoaderBuilder()
                            ...
                            .setPluginsSharedLibFolder(tempPluginsFolder)
                            .build();

                    List<ArtifactPlugin> artifactPlugins = temporaryContextClassLoader
                            .getArtifactPluginClassLoaders()
                            .stream()
                            .map(artifactClassLoader -> new DefaultArtifactPlugin(this.artifactPluginDescriptors.stream()
                                                                     .filter(artifactDescriptor -> artifactDescriptor.getName().equals(artifactDescriptor.getName()))
                                                                     .findFirst().get(),
                                                                              artifactClassLoader)).collect(toList());

                    artifactMuleContextBuilder = new ArtifactMuleContextBuilder()
                            .setArtifactType(DOMAIN)
                            .setArtifactPlugins(artifactPlugins)
                            .setExecutionClassloader(temporaryContextClassLoader)
