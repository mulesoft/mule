 ...
 * <p/>
 * Resolution process consist if find, for each service provider, all the service dependencies it has.
 * Then, service providers will be instantiated when the required set of dependencies is available.
 * <p/>
 * Resolution of services providers must be done always in the same order for the same set of providers
 * to ensure a consistent startup of the container.
 * <p/>
...
public class ReflectionServiceResolver implements ServiceResolver
{
    ...
    public List<Service> resolveServices(List<ServiceProvider> serviceProviders) throws ServiceResolutionError
    {
        ...
        while (continueResolution)
        {
            ...
            for (DependencyAwareServiceProvider dependencyAwareServiceProvider : unresolvedServiceProviders)
            {
                ...
                {
                    serviceProviderResolutionHelper.injectInstance(dependencyAwareServiceProvider.serviceProvider, registeredServices.values());
    ...
    private List<DependencyAwareServiceProvider> createDependencyAwareServiceProviders(List<ServiceProvider> serviceProviders)
    {
        ...
        {
            result.add(new DependencyAwareServiceProvider(serviceProvider, serviceProviderResolutionHelper.findServiceDependencies(serviceProvider)));
        }
