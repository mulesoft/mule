...
public class AuthenticationCodeAuthenticateTestCase extends AbstractMuleTestCase
{
    ...
    private UserOAuthContext mockUserOAuthContext = Mockito.mock(UserOAuthContext.class, RETURNS_DEEP_STUBS.get());
    private ExpressionManager mockExpressionManager = Mockito.mock(ExpressionManager.class, RETURNS_DEEP_STUBS.get());
    private HttpRequestBuilder mockHttpRequesBuilder = Mockito.mock(HttpRequestBuilder.class, RETURNS_DEEP_STUBS.get());
    ...
    public void processWithoutUserIdConfigured() throws Exception
    {
        ...
        when(mockAuthorizationCodeConfig.getUserOAuthContext().getContextForUser(UserOAuthContext.DEFAULT_RESOURCE_OWNER_ID)).thenReturn(mockUserOAuthContext);
        when(mockUserOAuthContext.getAccessToken()).thenReturn(ACCESS_TOKEN);
        authenticationCodeAuthenticate.authenticate(mockMuleEvent, mockHttpRequesBuilder);
        verify(mockHttpRequesBuilder).addHeader(HttpHeaders.Names.AUTHORIZATION, buildAuthorizationHeaderContent(ACCESS_TOKEN));
    }
    ...
    public void processWithoutUserIdConfiguredAndNoAccessToken() throws Exception
    {
        ...
        when(mockAuthorizationCodeConfig.getUserOAuthContext().getContextForUser(UserOAuthContext.DEFAULT_RESOURCE_OWNER_ID)).thenReturn(mockUserOAuthContext);
        when(mockUserOAuthContext.getAccessToken()).thenReturn(null);
    ...
    @Test
    public void processWithNullResourceOwnerIdExpression() throws Exception
    {
        authenticationCodeAuthenticate.setResourceOwnerId("#[null]");
        configureAuthenticationCodeAuthenticate();
    ...
    @Test
    public void shouldRetryRequestCustomResourceOwnerId() throws Exception
    {
    ...
    private void shouldRetryRequestForUser(String resourceOwnerId) throws Exception
    {
        when(mockExpressionManager.evaluate(anyString(), any(MuleEvent.class))).thenReturn(true);
        if (resourceOwnerId != null)
        {
            authenticationCodeAuthenticate.setResourceOwnerId(resourceOwnerId);
        }
        when(mockAuthorizationCodeConfig.getRefreshTokenWhen()).thenReturn("someValue");
        configureAuthenticationCodeAuthenticate();
        assertThat(authenticationCodeAuthenticate.shouldRetry(mockMuleEvent), Is.is(true));
        verify(mockAuthorizationCodeConfig).refreshToken(mockMuleEvent, resourceOwnerId == null ? UserOAuthContext.DEFAULT_RESOURCE_OWNER_ID : resourceOwnerId);
    }
