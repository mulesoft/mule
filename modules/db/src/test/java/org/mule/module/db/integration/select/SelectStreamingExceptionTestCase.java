...
public class SelectStreamingExceptionTestCase extends AbstractDbIntegrationTestCase
{
    ...
    {
        GenericDbConfig config = muleContext.getRegistry().get("pooledJdbcConfig");
    ...
    public void streamingException() throws Exception
    {
        for (int i = 0; i < POOL_CONNECTIONS + 1; ++i)
        {
            ...
            {
                assertTrue("Iteration " + i, e.getMessage().startsWith("Table/View 'NOT_EXISTS' does not exist. (java.sql.SQLSyntaxErrorException)."));
            }
        }
    }
