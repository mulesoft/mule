...
/**
 * A {@link MacroExpansionModuleModel} works tightly with a
 * {@link ApplicationModel} to go over all the registered
 * {@link ExtensionModel}s that are XML based (see
 * {@link XmlExtensionModelProperty}) looking for code to macro expand.
 * <p/>
 * For every occurrence that happens, it will expand the operations.
 * <p/>
 * This object works by handling {@link ComponentModel}s directly, consuming the
 * {@link GlobalElementComponentModelModelProperty} for the "config" elements
 * while the {@link OperationComponentModelModelProperty} for the operations
...
public class MacroExpansionModuleModel {
	 ...
	 * If the module's name is math, then the value of this field will name the
	 * global element as <math:config ../>
	 */
	public static final String MODULE_CONFIG_GLOBAL_ELEMENT_NAME = "config";

	/**
	 * literal that represents the name of the connection element for any given
	 * module. If the module's name is github, then the value of this field will
	 * name the global element as <github:connection ../>. As an example, think of
	...
	/**
	 * Used to obtain the {@link ComponentIdentifier} element from the <module/>'s
	 * original {@ink ComponentModel} to be later added in the macro expanded
	 * element (aka: <module-operation-chain ../>) so that the location set by the
	 * {@link org.mule.runtime.config.internal.dsl.model.ComponentLocationVisitor}
	...
	/**
	 * Reserved prefix in a <module/> to define a reference an operation of the same
	 * module (no circular dependencies allowed)
	 */
	...
	/**
	 * Used when the <module/> contains global elements without <property/>ies to be
	 * expanded, thus the macro expansion will take care of the default global
	 * elements macro expanding them ONCE, and replacing the
	 * {@link #MODULE_OPERATION_CONFIG_REF} in the <operation/>'s <body/>
	 * accordingly.
	 */
	...
	/**
	 * From a mutable {@code applicationModel}, it will store it to apply changes
	 * when the {@link #expand()} method is executed.
	 *
	 * @param applicationModel
	 *            to modify given the usages of elements that belong to the
	 *            {@link ExtensionModel}s contained in the {@code extensions} map.
	 * @param extensionModel
	 *            the {@link ExtensionModel}s to macro expand in the parameterized
	...
	public void expand() {
		...
		final Set<String> moduleGlobalElementsNames = moduleGlobalElements.stream()
				.map(ComponentModel::getNameAttribute).collect(toSet());
	...
	private void expandOperations(Set<String> moduleGlobalElementsNames) {
		applicationModel.executeOnEveryMuleComponentTree(containerComponentModel -> containerComponentModel
				.getInnerComponents().stream().filter(operationRefModel -> operationRefModel.getIdentifier()
						.getNamespace().equals(extensionModel.getXmlDslModel().getPrefix()))
				.forEach(operationRefModel -> {
					operationRefModel.getModel(OperationModel.class).ifPresent(operationModel -> {
						final String containerName = calculateContainerRootName(containerComponentModel,
								operationModel);
						final ComponentModel moduleOperationChain = createModuleOperationChain(operationRefModel,
								operationModel, moduleGlobalElementsNames, empty(), containerName);
	...
	/**
	 * Returns the rootest flow/subflow/munit:test/etc.'s name for the module chain
	 * that will be macro expanded. By default, it will assume it's a flow or even
	 * an already macro expanded element, but if not it will ask for the parent
	 * component model, making any scope (such as foreach, async, etc.) look for the
	 * flow in which is contained.
	 *
	 * @param containerComponentModel
	 *            container element to look for the root element that contains it
	 * @param operationModel
	 *            the operation just to log if something went bad
	 * @return the name of the root element that contains the
	 *         {@code containerComponentModel}. Not null.
	 * @throws MuleRuntimeException
	 *             if it cannot find the root element. It should never happen, as
	 *             the macro expansion for operations ONLY happens when being
	 *             consumed from within a flow/subflow/etc.
	 */
	private String calculateContainerRootName(ComponentModel containerComponentModel, OperationModel operationModel) {
		...
		if (containerComponentModel.isRoot()) {
			...
			throw new MuleRuntimeException(createStaticMessage(format(
					"Should have not reach here. There was no root container element while doing the macro expansion for the module [%s], operation [%s]",
	...
	private void expandGlobalElements(List<ComponentModel> moduleComponentModels,
	...
	private void addDefaultGlobalElements(Set<String> moduleGlobalElementsNames) {
		...
		List<ComponentModel> globalElements = extensionModel
				.getModelProperty(GlobalElementComponentModelModelProperty.class).get().getGlobalElements();
		...
		ComponentModel configRefModel = new ComponentModel.Builder()
				.setIdentifier(ComponentIdentifier.builder()
						...
						.namespace(extensionModel.getXmlDslModel().getPrefix()).name(DEFAULT_GLOBAL_ELEMENTS).build())
				.build();
		...
		globalElements.forEach(globalElementComponenModel -> {
			final ComponentModel macroExpandedImplicitGlobalElement = copyGlobalElementComponentModel(
					globalElementComponenModel, defaultGlobalElementSuffix, moduleGlobalElementsNames, new HashMap<>());
	...
	private void macroExpandGlobalElements(List<ComponentModel> moduleComponentModels,
			Set<String> moduleGlobalElementsNames) {
		// scenario where it will macro expand as many times as needed all the
		// references of the smart connector configurations
		applicationModel.executeOnEveryMuleComponentTree(muleRootComponentModel -> {
			muleRootComponentModel
					.getInnerComponents().stream().filter(configRefModel -> configRefModel.getIdentifier()
							.getNamespace().equals(extensionModel.getXmlDslModel().getPrefix()))
						...
						((ComponentAst) configRefModel).getModel(ConfigurationModel.class)
								.ifPresent(configurationModel -> {
									Map<String, String> propertiesMap = ((ComponentAst) configRefModel).getParameters()
											.stream().collect(toMap(paramAst -> paramAst.getModel().getName(),
													paramAst -> (String) paramAst.getValue().getRight()));
									Map<String, String> connectionPropertiesMap = extractConnectionProperties(
											(ComponentAst) configRefModel, configurationModel);
									propertiesMap.putAll(connectionPropertiesMap);
									final Map<String, String> literalsParameters = getLiteralParameters(propertiesMap,
											emptyMap());
									List<ComponentModel> replacementGlobalElements = createGlobalElementsInstance(
											configRefModel, moduleComponentModels, moduleGlobalElementsNames,
											literalsParameters);
	...
	private List<ComponentModel> createGlobalElementsInstance(ComponentModel configRefModel,
			List<ComponentModel> moduleGlobalElements, Set<String> moduleGlobalElementsNames,
		...
		return moduleGlobalElements.stream().map(globalElementModel -> {
			final ComponentModel macroExpandedGlobalElement = copyGlobalElementComponentModel(globalElementModel,
					configRefModel.getNameAttribute(), moduleGlobalElementsNames, literalsParameters);
	...
	private List<ComponentModel> getModuleGlobalElements() {
		return extensionModel.getModelProperty(GlobalElementComponentModelModelProperty.class)
				.map(GlobalElementComponentModelModelProperty::getGlobalElements).orElse(new ArrayList<>());
	}

	/**
	 * Takes a one liner call to any given message processor, expand it to creating
	 * a "module-operation-chain" scope which has the set of properties, the set of
	 * parameters and the list of message processors to execute.
	 *
	 * @param operationRefModel
	 *            message processor that will be replaced by a scope element named
	 *            "module-operation-chain".
	 * @param operationModel
	 *            operation that provides both the <parameter/>s and content of the
	 *            <body/>
	 * @param moduleGlobalElementsNames
	 *            collection with the global components names (such as <http:config
	 *            name="a"../>, <file:config name="b"../>, <file:matcher
	 *            name="c"../> and so on) that are contained within the <module/>
	 *            that will be macro expanded
	 * @param configRefParentTnsName
	 *            parent reference to the global element if exists (it might not be
	 *            global elements in the current module). Useful when replacing
	 *            {@link #TNS_PREFIX} operations, as the references to the global
	 *            elements will be those of the rootest element of the operations
	 *            consumed by the app.
	 * @param containerName
	 *            name of the container that contains the operation to be macro
	 *            expanded. Not null nor empty.
	 * @return a new component model that represents the old placeholder but
	 *         expanded with the content of the <body/>
	 */
	private ComponentModel createModuleOperationChain(ComponentModel operationRefModel, OperationModel operationModel,
			Set<String> moduleGlobalElementsNames, Optional<String> configRefParentTnsName, String containerName) {
		final OperationComponentModelModelProperty operationComponentModelModelProperty = operationModel
				.getModelProperty(OperationComponentModelModelProperty.class).get();
		...
		operationRefModel.getRawParameters().forEach((paramName, paramValue) -> {
			processorChainBuilder.addParameter(paramName, paramValue,
					operationRefModel.isParameterValueProvidedBySchema(paramName));
		});
		...
		bodyProcessors.stream().map(bodyProcessor -> lookForTNSOperation((ComponentAst) bodyProcessor)
				...
				.orElseGet(() -> copyOperationComponentModel(bodyProcessor, configRefName, moduleGlobalElementsNames,
						getLiteralParameters(propertiesMap, parametersMap), containerName)))
				.forEach(processorChainBuilder::addChildComponentModel);

		copyErrorMappings(operationRefModel, processorChainBuilder);

		for (Map.Entry<String, Object> customAttributeEntry : operationRefModel.getMetadata().getParserAttributes()
				.entrySet()) {
			processorChainBuilder.addCustomAttribute(customAttributeEntry.getKey(), customAttributeEntry.getValue());
		}
		for (Map.Entry<String, String> customAttributeEntry : operationRefModel.getMetadata().getDocAttributes()
				.entrySet()) {
			processorChainBuilder.addCustomAttribute(customAttributeEntry.getKey(), customAttributeEntry.getValue());
		}
	...
	/**
	 * Looks for the value of the {@link #MODULE_OPERATION_CONFIG_REF} in the
	 * current <operation/>, if not found then tries to fallback to the default
	 * global element name. See {@link #defaultGlobalElementName()} method.
	 *
	 * @param operationRefModel
	 *            <operaton/> to lookup the expected string reference, if exists.
	 * @return the suffix needed to be used when macro expanding elements, or
	...
	/**
	 * If the current operation contains any {@link ApplicationModel#ERROR_MAPPING}
	 * as a child, it will copy them to the macro expanded <module-operation-chain/>
	 * as childs after the list of message processors.
	 *
	 * @param operationRefModel
	 *            {@link ComponentModel} to look for the possible child elements
	 *            {@link ApplicationModel#ERROR_MAPPING_IDENTIFIER}
	 * @param processorChainBuilder
	 *            the <module-operation-chain/> where the errors mappings will be
	 *            copied to
	 */
	private void copyErrorMappings(ComponentModel operationRefModel, ComponentModel.Builder processorChainBuilder) {
		operationRefModel.getInnerComponents().stream().filter(
				componentModel -> componentModel.getIdentifier().equals(ApplicationModel.ERROR_MAPPING_IDENTIFIER))
	 ...
	 *
	 * @param modelToCopy
	 *            original source of truth that comes from the <module/>
	 * @return a transformed {@link ComponentModel} from the {@code modelToCopy},
	 *         where the element's attributes has been updated accordingly (both
	 *         global components updates plus the line number, and so on). If the
	...
	/**
	 * @param propertiesMap
	 *            <property>s that are feed in the current usage of the <module/>
	 * @param parametersMap
	 *            <param>s that are feed in the current usage of the <module/>
	 * @return a {@link Map} of <property>s and <parameter>s that could be replaced
	 *         by their literal values
	 */
	private Map<String, String> getLiteralParameters(Map<String, String> propertiesMap,
			Map<String, String> parametersMap) {
		final Map<String, String> literalsParameters = propertiesMap.entrySet().stream()
				.filter(entry -> !isExpression(entry.getValue()))
				.collect(toMap(e -> getReplaceableExpression(e.getKey(), VARS), Map.Entry::getValue));

		literalsParameters.putAll(parametersMap.entrySet().stream().filter(entry -> !isExpression(entry.getValue()))
				.collect(toMap(e -> getReplaceableExpression(e.getKey(), VARS), Map.Entry::getValue)));
		return literalsParameters;
	}

	/**
	 * Assembly an expression to validate if the macro expansion of the current
	 * <module> can be directly replaced by the literals value
	 *
	 * @param name
	 *            of the parameter (either a <property> or a <parameter>)
	 * @param prefix
	 *            binding to append for the expression to be replaced in the
	 *            <module>'s code
	 * @return the expression that access a variable through a direct binding (aka:
	 *         a "static expression", as it doesn't use the {@link CoreEvent})
	 */
	...
	/**
	 * Extracts the properties of the current <module/> if applies (it might not
	 * have a configuration in it)
	 *
	 * @param configRefName
	 *            current <operation/> to macro expand, from which the config-ref
	 *            attribute's value will be extracted.
	...
	private Map<String, String> extractProperties(Optional<String> configRefName) {
		...
		configRefName.filter(configParameter -> defaultGlobalElementName()
				.map(defaultGlobalElementName -> !defaultGlobalElementName.equals(configParameter)).orElse(true))
				.ifPresent(configParameter -> {
					// look for the global element which "name" attribute maps to "configParameter"
					// value
					// or a nested element to a config that was added by the macroexpansion of
					// another module before
					...
					configRefComponentModel.getParameters().stream().filter(paramAst -> paramAst.getRawValue() != null)
	...
	/**
	 * If the current {@link ExtensionModel} does have a
	 * {@link ConnectionProviderModel}, then it will check if the current XML does
	 ...
	 *
	 * @param configRefComponentModel
	 *            root element of the current XML config (global element of the
	 *            parameterized operation)
	 * @param configurationModel
	 *            configuration model of the current element
	...
	private Map<String, String> extractConnectionProperties(ComponentAst configRefComponentModel,
		...
		return configurationModel.getConnectionProviderModel(MODULE_CONNECTION_GLOBAL_ELEMENT_NAME)
				.flatMap(
						connectionProviderModel -> configRefComponentModel.directChildrenStream()
	...
	/**
	 * Goes over the {@code modelToCopy} by consuming the attributes as they are,
	 * unless some of them are actually targeting a global component (such as a
	 * configuration), in which it will append the {@code configRefName} to that
	 * reference, which will be the definitive name once the Mule application has
	 * been completely macro expanded in the final XML configuration.
	 *
	 * @param modelToCopy
	 *            original source of truth that comes from the <module/>
	 * @param configRefName
	 *            name of the configuration being used in the Mule application
	 * @param moduleGlobalElementsNames
	 *            names of the <module/>s global component that will be macro
	 *            expanded in the Mule application
	 * @param literalsParameters
	 *            {@link Map} with all he <property>s and <parameter>s that were
	 *            feed with a literal value in the Mule application's code.
	 * @return a transformed {@link ComponentModel} from the {@code modelToCopy},
	 *         where the global element's attributes has been updated accordingly
	 *         (both global components updates plus the line number, and so on). If
	 *         the value for some parameter can be optimized by replacing it for the
	 *         literal's value, it will be done as well using the
	...
	private ComponentModel copyGlobalElementComponentModel(ComponentModel modelToCopy, String configRefName,
			Set<String> moduleGlobalElementsNames, Map<String, String> literalsParameters) {
		...
		for (ComponentModel operationChildModel : modelToCopy.getInnerComponents()) {
			globalElementReplacementModel.addChildComponentModel(copyGlobalElementComponentModel(operationChildModel,
					configRefName, moduleGlobalElementsNames, literalsParameters));
		}
	 ...
	 *
	 * @param modelToCopy
	 *            original <operation/> source of truth that comes from the
	 *            <module/>
	 * @param configRefName
	 *            name of the configuration being used by the current <operation/>.
	 *            If the operation is a TNS one, then it has the value of the
	 *            rootest <operation/> being called from the application.
	 * @param moduleGlobalElementsNames
	 *            names of the <module/>s global component that will be macro
	 *            expanded in the Mule application
	 * @param literalsParameters
	...
	private ComponentModel copyOperationComponentModel(ComponentModel modelToCopy, Optional<String> configRefName,
			Set<String> moduleGlobalElementsNames, Map<String, String> literalsParameters, String containerName) {
		ComponentModel.Builder operationReplacementModel = getComponentModelBuilderFrom(modelToCopy);
		for (Map.Entry<String, String> entry : modelToCopy.getRawParameters().entrySet()) {
		...
		modelToCopy.getInnerComponents().stream()
				.map(operationChildModel -> lookForTNSOperation((ComponentAst) operationChildModel)
						.map(tnsOperation -> createModuleOperationChain(operationChildModel, tnsOperation,
								moduleGlobalElementsNames, configRefName, containerName))
						.orElseGet(() -> copyOperationComponentModel(operationChildModel, configRefName,
								moduleGlobalElementsNames, literalsParameters, containerName)))
	...
	private ComponentModel.Builder getComponentModelBuilderFrom(ComponentModel componentModelOrigin) {
		...
		operationReplacementModel.setIdentifier(componentModelOrigin.getIdentifier())
	...
	private ComponentModel buildFrom(ComponentModel componentModelOrigin,
			ComponentModel.Builder operationReplacementModel) {
	...
	/**
	 * True if an <operation/> A calls an <operation/> B defined in the same
	 * <module/> by using <tns:B .../>
	 *
	 * @param operationComponentModel
	 *            operation that might or might not be referencing operations of the
	 *            same module.
	 * @return true if it's an <operation/> reference in the same <module/>, false
	 *         otherwise
	 */
	...
	/**
	 * Looks for an operation exposed in the current {@link ExtensionModel} that's
	 * being targeted by other operation through the {@link #TNS_PREFIX} prefix.
	 *
	 * @param componentModel
	 *            to check whether targets a <module/>'s operation or not.
	 * @return an {@link OperationModel} if the parameterized {@code componentModel}
	 *         targets an <operation/> of the same module by using the
	...
	// TODO MULE-9849: until there's no clear way to check against the
	// ComponentModel using the
	// org.mule.runtime.config.dsl.processor.AbstractAttributeDefinitionVisitor.onReferenceSimpleParameter(),
	// we workaround
