...
/**
 * Repository for storing the dynamic class generated to mimic {@link org.springframework.beans.factory.FactoryBean}
 * from an {@link ObjectFactory}. This is done because we need dependency injection and instrospection done over the
 * {@link ObjectFactory} without the user knowing about it.
 * <p>
 * The created {@link org.springframework.beans.factory.FactoryBean} is the one that receives the injection of fields
 * declared by the {@link ObjectFactory}. It also provides information about the instance type that is creating since it's
 * used to know the order in which beans must be initialised based on the dependencies between them.
 * <p>
 * The repository has a cache of the created classes based on the configuration of the {@link ComponentBuildingDefinition}
