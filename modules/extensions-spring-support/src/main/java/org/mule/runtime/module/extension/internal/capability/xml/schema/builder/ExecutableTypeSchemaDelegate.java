...
abstract class ExecutableTypeSchemaDelegate {
  ...
  private void generateNestedRouteElement(ExtensionType type, DslElementSyntax routeDsl, NestedRouteModel routeModel) {
    if (supportsMultiple(routeModel)) {
      ...
      LocalComplexType routesWrapperElementType =
          builder.getObjectSchemaDelegate().createTypeExtension(MULE_ABSTRACT_EXTENSION_TYPE);
      routesWrapperElement.setComplexType(routesWrapperElementType);

      final ExtensionType childRoutesType = new ExtensionType();
      initialiseSequence(childRoutesType);
      doGenerateNestedRouteElement(childRoutesType, routeDsl.getChilds().get(0), routeModel, "unbounded");

      routesWrapperElementType.getComplexContent().getExtension().setSequence(childRoutesType.getSequence());
      type.getSequence().getParticle().add(objectFactory.createElement(routesWrapperElement));
    } else {
      doGenerateNestedRouteElement(type, routeDsl, routeModel, MAX_ONE);
    }
  }

  private void doGenerateNestedRouteElement(ExtensionType type, DslElementSyntax routeDsl, NestedRouteModel routeModel,
                                            String maxOccurs) {
