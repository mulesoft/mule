...
public enum Http2Status {
  ...
  public static String getReasonPhraseForStatusCode(int statusCode) {
    final Http2Status statusByCode = getStatusByCode(statusCode);
    return statusByCode == null ? null : statusByCode.getReasonPhrase();
  }

  /**
   * Returns an {@link Http2Status} corresponding to a given status code.
   *
   * @param statusCode the HTTP status code to search for
   * @return an {@link Http2Status} representing {@code statusCode} or null if not found
   */
  public static Http2Status getStatusByCode(int statusCode) {
