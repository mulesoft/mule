...
public class MailMessageReceiver extends PollingMessageReceiver implements MessageCountListener
{
    ...
    {
        if(folder==null) throw new IllegalArgumentException("Mail folder cannot be null");
        this.folder = folder;
        synchronized (this.folder)
        {
            if (!this.folder.isOpen())
            {
                try
                {
                    //Depending on Server implementation it's not always necessary
...
//            {
//                logger.warn("Failed to close folder: " + folder.getFullName(), me);
//            }
            }
        }
    }
     ...
     * @throws IOException        If a failiure happens writing the message
     * @throws MessagingException If a failiure happens reading the message
     */
