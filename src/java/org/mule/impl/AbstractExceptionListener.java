...
public abstract class AbstractExceptionListener implements ExceptionListener, Initialisable
{
     ...
     * This method should not be called if an event is routed from this exception
     * handler to an endpoint that should take part in the current transaction
     */
    ...
    /**
     * Routes the current exception to an error endpoint such as a Dead Letter Queue (jms)
     * This method is only invoked if there is a UMOMassage available to dispatch. The message
     * dispatched from this method will be an <code>ExceptionMessage</code> which contains the
    ...
    /**
     * Returns an endpoint for the given exception. ExceptionListeners can
     * have multiple endpoints registered on them.  This methods allows custom
     ...
     * @return The endpoint used to dispatch an exception message on or null if there are
    ...
    /**
     * Logs a fatal error message to the logging system.  This should be used mostly if an error
     * occurs in the exception listener itself.
     * This implementation logs the the message itself to the logs if it is not null
     ...
     * A <code>MessagingException</code> holds a reference to the current message that is passed into
     * this method
     * @param message the current message being processed
     * @param e the top level exception thrown.  This may be a Messaging exception or some
     * wrapper exception
    ...
    /**
     * A routing exception is thrown when an excpetion occurs during normal message processing
     * A <code>RoutingException</code> holds a reference to the current message and te endpoint
     * being routing to or from when the error occurred.  Both are passed into this method
     * @param message the current message being processed
     * @param endpoint  the endpoint being dispatched to or received from when the error
     * occurred
     * @param e the top level exception thrown.  This may be a Messaging exception or some
     * wrapper exception
    ...
    /**
     * Lifecycle exceptions are thrown when an error occurs during an object's lifecycle call
     * such as start, stop or initialise.  The exception contains a reference to the object
     * that failed which can be used for more informative logging.
     * @param component the object that failed during a lifecycle call
     * @param e the top level exception thrown.  This may or may not be the <code>LifecycleException</code>
     * but a lifecycle exception will be present in the exception stack.
