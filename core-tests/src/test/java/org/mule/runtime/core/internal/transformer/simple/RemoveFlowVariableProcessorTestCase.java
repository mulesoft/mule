...
public class RemoveFlowVariableProcessorTestCase extends AbstractRemoveVariablePropertyProcessorTestCase {
  ...
  @Override
  protected void addMockedPropeerties(InternalEvent mockEvent, Set<String> properties) {
    when(mockEvent.getVariables().keySet()).thenReturn(properties);
  }

  @Override
  protected void verifyRemoved(InternalEvent mockEvent, String key) {
    assertThat(mockEvent.getVariables().keySet(), not(contains(key)));
  }

  @Override
  protected void verifyNotRemoved(InternalEvent mockEvent, String key) {
