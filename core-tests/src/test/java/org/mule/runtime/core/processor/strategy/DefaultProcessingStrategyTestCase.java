...
public class DefaultProcessingStrategyTestCase extends ProactorProcessingStrategyTestCase {
  ...
  public void tx() throws Exception {
    ...
    assertThat(threads.stream().filter(name -> name.startsWith(ThreadType.CPU_LIGHT.name())).count(), equalTo(0l));
    assertThat(threads.stream().filter(name -> name.startsWith(ThreadType.IO.name())).count(), equalTo(0l));
    assertThat(threads.stream().filter(name -> name.startsWith(ThreadType.CPU_INTENSIVE.name())).count(), equalTo(0l));
  }
