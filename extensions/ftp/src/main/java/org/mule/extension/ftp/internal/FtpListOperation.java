...
/**
 * Contains an operation which allows to list files using an optional {@link FtpFilePredicateBuilder}
 *
 * @since 4.0
 */
public class FtpListOperation
{
    ...
    public TreeNode list(@Connection FileSystem fileSystem,
                         ...
                         MuleMessage<?, ?> message,
                         @Optional FtpFilePredicateBuilder matcher)
    {
        return fileSystem.list(directoryPath,
                               recursive,
                               message,
                               getPredicate(matcher));
    }
