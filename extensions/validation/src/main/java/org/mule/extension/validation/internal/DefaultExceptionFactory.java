 ...
 * which receives a single argument assignable from
...
public class DefaultExceptionFactory implements ExceptionFactory
{
    ...
    @Override
    public <T extends Exception> T createException(ValidationResult result, Class<T> exceptionClass)
    {
        ConstructorDelegate<T> constructorDelegate = (ConstructorDelegate<T>) get(constructorCache, exceptionClass);
        try
        {
            return constructorDelegate.createException(result, result.getMessage());
        }
        catch (Exception e)
        {
    ...
    @Override
    public Exception createException(ValidationResult result, String exceptionClassName)
    {
        return createException(result, get(classCache, exceptionClassName));
    }
    ...
    {
        Collection<Constructor> candidate = getConstructors(exceptionType, withParameters(ValidationResult.class));
        ...
        if (CollectionUtils.isEmpty(candidate))
        {
            throw new IllegalArgumentException(
                    String.format(
                            "Exception type %s was expected to contain a constructor with a single String argument but a matching constructor " +
                            "could not be found.",
    ...
    private interface ConstructorDelegate<T extends Exception>
    {
        ...
        T createException(ValidationResult validationResult, String message) throws Exception;
    }
    ...
    private class ValidationResultConstructorDelegate<T extends Exception> implements ConstructorDelegate<T>
    {
        ...
        @Override
        public T createException(ValidationResult validationResult, String message) throws Exception
        {
