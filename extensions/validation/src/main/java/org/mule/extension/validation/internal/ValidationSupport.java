...
abstract class ValidationSupport
{
    ...
    protected void validateWith(Validator validator, ValidationContext validationContext, MuleEvent event) throws Exception
    {
        ...
        if (result.isError())
        {
            ...
            {
                throw config.getExceptionFactory().createException(result, ValidationException.class);
            }
            else
            {
                throw config.getExceptionFactory().createException(result, customExceptionClass);
            }
        }
    }
