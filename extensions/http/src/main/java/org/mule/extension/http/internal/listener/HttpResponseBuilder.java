...
import org.mule.extension.http.internal.HttpParamType;
import org.mule.extension.http.internal.multipart.HttpMultipartEncoder;
import org.mule.extension.http.internal.multipart.HttpPartDataSource;
...
public class HttpResponseBuilder extends HttpMessageBuilder implements Initialisable, MuleContextAware {
  ...
  private boolean supportsTransferEncoding(Event event) {
    String httpVersion = ((HttpRequestAttributes) event.getMessage().getAttributes()).getVersion();
  ...
  private boolean isNotIgnoredProperty(String outboundPropertyName) {
    return !outboundPropertyName.startsWith(HTTP + ".") && !outboundPropertyName.equalsIgnoreCase(CONNECTION)
