...
public class InboundEndpointMimeTypeCheckingMessageProcessor implements MessageProcessor
{
    ...
    public MuleEvent process(MuleEvent event) throws MessagingException
    {
        ...
        if (endpointMimeType != null)
        {
            ...
            String contentType = message.getProperty(CONTENT_TYPE_PROPERTY, PropertyScope.INBOUND);
            if (contentType == null)
            {
                contentType = message.getProperty(CONTENT_TYPE_PROPERTY, PropertyScope.OUTBOUND);
            }
            if (contentType == null)
            {
                message.setProperty(CONTENT_TYPE_PROPERTY, endpointMimeType, PropertyScope.INBOUND);
            }
            else
            {
                try
