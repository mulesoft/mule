...
/**
 * Specialized {@link org.mule.processor.BlockingProcessorExecutor} that pauses iteration in the case a {@link org.mule.processor.NonBlockingMessageProcessor}
 * is invoked and the flow is executing using non-blocking.  Processor execution is then continued when the {@link org.mule.processor.NonBlockingMessageProcessor}
        ...
        if (event.isAllowNonBlocking())
        {
            ...
            if (processor instanceof NonBlockingMessageProcessor)
            {
                event = new DefaultMuleEvent(event, new NonBlockingResponseReplyToHandler(new CompletionHandler<MuleEvent, MessagingException>()
                {
                    ...
                    {
                        NonBlockingProcessorExecutor.this.event = new DefaultMuleEvent(o, replyToHandler);
                        try
                        {
                            resume();
                        }
