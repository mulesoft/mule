...
public interface UMOContainerContext extends Initialisable, Disposable
{
     ...
     * provide a default name that users can choose to override The name can be
     * used to reference a container when more than one is registered
     * 
     ...
     * that is embedded in the &lt;configuration&gt; element of the
     * &lt;container-context$gt; element.
     * 
     * @param configuration
     * @param doctype the doctype declaration to use for the configuration
     *            fragment. can be null if no validation is to be performed or
     *            the fragment is not Xml
     * @param encoding the encoding to use in the Xml declaration. Default is
     *            UTF-8
