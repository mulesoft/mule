...
public interface ConnectionManagerAdapter extends ConnectionManager, Lifecycle {
  ...
  /**
   * Returns the {@link DefaultReconnectionConfig} that should be applied to the given {@code connectionProvider}
   *
   * @param connectionProvider a {@link ConnectionProvider}
   * @param <C>                the generic type of the connection returned by the provider
   * @return a {@link DefaultReconnectionConfig}
   */
  <C> DefaultReconnectionConfig getReconnectionConfigFor(ConnectionProvider<C> connectionProvider);
