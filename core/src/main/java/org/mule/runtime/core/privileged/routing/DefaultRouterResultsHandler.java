...
public class DefaultRouterResultsHandler implements RouterResultsHandler {
  ...
  /**
   * Aggregates the events in the results list into one single {@link BaseEvent} You should only use this
  ...
  public BaseEvent aggregateResults(final List<BaseEvent> results,
    ...
    if (results == null) {
      ...
      List<BaseEvent> nonNullResults = results.stream().filter(object -> object != null &&
          object.getMessage() != null).collect(toList());

      if (nonNullResults.size() == 0) {
        return BaseEvent.builder(previous).message(Message.of(null)).build();
