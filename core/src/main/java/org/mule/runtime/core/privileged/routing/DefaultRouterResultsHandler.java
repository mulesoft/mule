...
public class DefaultRouterResultsHandler implements RouterResultsHandler {
  ...
  /**
   * Aggregates the events in the results list into one single {@link InternalEvent} You should only use this
  ...
  public InternalEvent aggregateResults(final List<InternalEvent> results,
                                        final InternalEvent previous) {
    if (results == null) {
      return null;
      ...
      List<InternalEvent> nonNullResults = results.stream().filter(object -> object != null &&
  ...
  private InternalEvent createMessageCollection(final List<InternalEvent> nonNullResults,
                                                final InternalEvent previous) {
    final Builder resultBuilder = InternalEvent.builder(previous);
