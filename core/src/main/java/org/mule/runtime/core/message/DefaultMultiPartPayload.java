...
public class DefaultMultiPartPayload implements Serializable, MultiPartPayload
{
    ...
    {
        return parts.stream()
                    .filter(m -> partName.equals(getPartAttributes(m).getName()))
                    .findFirst()
                    .get();
    }
    ...
    {
        return parts.stream()
                    .filter(m -> BODY_PART_NAME.equals(getPartAttributes(m).getName()))
                    .findFirst()
                    .orElse(null);
    }
    ...
    {
        return parts.stream()
                    .anyMatch(m -> BODY_PART_NAME.equals(getPartAttributes(m).getName()));
    }
