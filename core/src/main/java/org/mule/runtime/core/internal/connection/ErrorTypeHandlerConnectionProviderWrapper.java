...
/**
 * {@link ConnectionProviderWrapper} implementation which handles the exceptions occurred when {@link this#connect()}
 * and the failed {@link ConnectionValidationResult} from the {@link this#validate(Object)}, this wrapper consumes these
 * outputs and if a {@link ErrorTypeDefinition} is provided transforms it and communicates the proper {@link ErrorType}
 *
 * @param <C>
 * @since 4.0
 */
public final class ErrorTypeHandlerConnectionProviderWrapper<C> extends ConnectionProviderWrapper<C> {
  ...
  /**
   * Delegates to the proper {@link ConnectionProvider}, if this fails and throws a {@link ConnectionException},
   * this method will introspect into the cause of this exception and if the cause is a {@link ModuleException}
   * a new {@link ConnectionException} will be created communicating the proper {@link ErrorType}, otherwise
   * the original exception will be propagated.
  ...
  /**
   * Delegates the validation of the connection {@link C} to the proper {@link ConnectionProvider}, is the validation
   * is not valid this method will introspect into the exception cause and look for a {@link ModuleException}, if one
   * is found a new {@link ConnectionValidationResult} will be created with the same information, but communicating
   * the {@link ErrorType}.
  ...
  private ComponentIdentifier getIdentifier(ErrorTypeDefinition errorType) {
    return ComponentIdentifier.builder().withName(errorType.getType()).withNamespace(namespace).build();
  }
}
