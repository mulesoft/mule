...
public class Correlation implements Serializable
{
     ...
     * <p>
     * The id is associated with the message using the underlying transport protocol. As such not all messages will
     * support the notion of a id i.e. tcp or file. In this situation the correlation Id is set as a property of the
     * message where it's up to developer to keep the association with the message. For example if the message is
     * serialised to xml the id will be available in the message.
     *
    ...
    /**
     * Gets the sequence or ordering number for this message in the the correlation group (as defined by
     * {@link #getId()}).
     *
