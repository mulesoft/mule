/*
 * Copyright (c) MuleSoft, Inc.  All rights reserved.  http://www.mulesoft.com
 * The software in this package is published under the terms of the CPAL v1.0
 * license, a copy of which has been included with this distribution in the
 * LICENSE.txt file.
 */

package org.mule.runtime.core.internal.profiling.tracing.event.span;

import org.mule.runtime.core.privileged.profiling.tracing.ChildSpanCustomizer;

public class DefaultChildSpanCustomizer implements ChildSpanCustomizer {

  private String childSpanName;

  public DefaultChildSpanCustomizer(String childSpanName) {
    this.childSpanName = childSpanName;
  }

  @Override
  public String getChildSpanName() {
    return childSpanName;
  }
}
