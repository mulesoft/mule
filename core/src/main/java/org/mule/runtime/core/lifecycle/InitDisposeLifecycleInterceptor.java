...
/**
 * A {@link LifecycleInterceptor} which tracks the completion of the {@link Initialisable}
 * phase so that if it fails to complete, the {@link Disposable} phase is only applied on
 * the target objects on which it could be successfully applied.
 *
 * @since 3.8
 */
public class InitDisposeLifecycleInterceptor implements LifecycleInterceptor
{
     ...
     * </ul>
     * <p>
     * If all three of the above conditions are met, then this method returns {@code false}.
    ...
    /**
     * If the {@code phase} name is {@link Initialisable#PHASE_NAME} then it tracks the
     * given {@code object} as initialised.
     *
    ...
    /**
     * If the {@code phase} name is {@link Initialisable#PHASE_NAME} then it
     * marks the {@link Initialisable} phase as completed and clears all other
