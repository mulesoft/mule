...
final class ConnectionManagementStrategyFactory
{
    ...
    private <C> ConnectionManagementStrategy<C> pooling(ConnectionProvider<C> connectionProvider)
    {
        ...
        return poolingProfile.isDisabled()
               ...
               : new PoolingConnectionManagementStrategy<>(connectionProvider,
                                                           poolingProfile,
                                                           (PoolingListener<C>) unwrap(connectionProvider),
                                                           muleContext);
    }
    ...
    private <C> ConnectionProvider<C> unwrap(ConnectionProvider<C> connectionProvider)
    {
        return connectionProvider instanceof ConnectionProviderWrapper
               ? unwrap(((ConnectionProviderWrapper) connectionProvider).getDelegate())
               : connectionProvider;
    }
}
