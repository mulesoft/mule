...
public final class ResultsToMessageList extends ResultsToMessageCollection implements List<Message> {
  ...
  public void sort(Comparator<? super Message> c) {
    delegate.sort((o1, o2) -> c.compare(toMessage(o1, mediaType, cursorProviderFactory, event),
                                        toMessage(o2, mediaType, cursorProviderFactory, event)));
  }

  @Override
  public Message get(int index) {
    return toMessage(delegate.get(index), mediaType, cursorProviderFactory, event);
  }

  @Override
  public Message set(int index, Message message) {
    Result previous = delegate.set(index, Result.builder(message).build());
    return previous != null ? toMessage(previous, mediaType, cursorProviderFactory, event) : null;
  }
  ...
  public ListIterator<Message> listIterator() {
    return new ResultToMessageListIterator(delegate.listIterator(), mediaType, cursorProviderFactory, event);
  }

  @Override
  public ListIterator<Message> listIterator(int index) {
    return new ResultToMessageListIterator(delegate.listIterator(index), mediaType, cursorProviderFactory, event);
  }

  @Override
  public List<Message> subList(int fromIndex, int toIndex) {
    return new ResultsToMessageList(delegate.subList(fromIndex, toIndex), mediaType, cursorProviderFactory, event);
  }
