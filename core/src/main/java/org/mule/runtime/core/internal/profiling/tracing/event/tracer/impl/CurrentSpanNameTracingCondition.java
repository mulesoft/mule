...
public class CurrentSpanNameTracingCondition implements TracingCondition {
  ...
  public void assertOnCurrentSpan(InternalSpan currentSpan) throws TracingConditionNotMetException {
    ...
    String parentSpanName = currentSpan.getName();

    if (!currentSpanExpectedName.equals(parentSpanName)) {
      throw new TracingConditionNotMetException("The current span has name: " + parentSpanName + ".  Expected a span with name: "
          + currentSpanExpectedName);
    }
  }
}
