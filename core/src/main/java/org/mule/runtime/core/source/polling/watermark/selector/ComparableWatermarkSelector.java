...
/**
 * Base class for {@link WatermarkSelector}s that evaluate {@link Comparable} objects. When a non {@link Comparable} value is
 * received, is is discarded.
 * 
 * @since 3.5.0
 */
public abstract class ComparableWatermarkSelector extends WatermarkSelector {
  ...
  /**
   * Returns an int value according to the {@link Comparable} contract (-1, 0, 1). Then the result of the comparation matches this
   * method's return value, then the selected value is updated
   */
  ...
  /**
   * A method used to normalize the value given back by the compareTo method. The Min and Max selectors expect Java's compareTo
   * method to return -1 or 1 respectively. However, this is not always the case, so we use this method change and negative result
   * to -1, and any positive result to 1.
   * 
