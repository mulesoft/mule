...
public class DefaultMuleContext implements MuleContext
{
    ...
    {
        ObjectStoreManager objectStoreManager = (ObjectStoreManager) registryBroker.lookupObject(MuleProperties.OBJECT_STORE_MANAGER);
        if (objectStoreManager == null)
        {
            Collection<ObjectStoreManager> temp = registryBroker.lookupObjects(ObjectStoreManager.class);
            if (temp.size() > 0)
            {
                objectStoreManager = temp.iterator().next();
            }
        }
