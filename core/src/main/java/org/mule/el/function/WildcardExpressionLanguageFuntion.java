...
{
	private final boolean DEFAULT_SENSITIVITY = true;
    ...
    public Object call(Object[] params, ExpressionLanguageContext context)
    {
        ...
        String wildcardPattern = verifyWilcardPattern(params[0]);
        ...
        if (numParams == 1)
        {
            try
            {
                result = isMatch(wildcardPattern,
                    context.getVariable("message", MessageContext.class).payloadAs(String.class), 
                    	DEFAULT_SENSITIVITY);
            }
    ...
    protected String verifyWilcardPattern(Object wilcardPattern)
    {
        if (wilcardPattern == null)
        {
            throw new IllegalArgumentException("wildcard pattern is null");
        }
        else if (!(wilcardPattern instanceof String))
        {
            throw new IllegalArgumentException("wildcard pattern is not a string");
        }
        return (String) wilcardPattern;
    }
    ...
    protected boolean isMatch(String wilcardPattern, String text, boolean caseSensitive)
    {
        return FilenameUtils.wildcardMatch(text, wilcardPattern, caseSensitive ? IOCase.SENSITIVE : IOCase.INSENSITIVE);
    }
