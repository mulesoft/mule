/*
 * $Id$
 * --------------------------------------------------------------------------------------
 * Copyright (c) MuleSoft, Inc.  All rights reserved.  http://www.mulesoft.com
 *
 * The software in this package is published under the terms of the CPAL v1.0
 * license, a copy of which has been included with this distribution in the
 * LICENSE.txt file.
 */

package org.mule.streaming;

import org.mule.api.MuleException;
import org.mule.api.streaming.ClosedConsumerException;
import org.mule.api.streaming.Consumer;
import org.mule.api.streaming.Producer;
import org.mule.tck.size.SmallTest;

import java.util.ArrayList;
import java.util.List;

import org.junit.Assert;
import org.junit.Before;
import org.junit.Test;
import org.mockito.Mockito;

@SmallTest
public class PagedBasedPagingConsumerTestCase extends AbstractPagingConsumerTestCase
{

    private Consumer<List<Integer>> consumer;
    private Producer<List<Integer>> producer;

    @Before
    public void setUp()
    {
        this.pages = this.getPages();
        this.producer = Mockito.spy(new TestProducer());
        this.consumer = Mockito.spy(new PagedBasedPagingConsumer<List<Integer>>(this.producer));
    }

    @Test(expected = ClosedConsumerException.class)
    public void happyPath() throws Exception
    {
        List<List<Integer>> elements = new ArrayList<List<Integer>>();

        while (!this.consumer.isConsumed())
        {
            elements.add(this.consumer.consume());
        }

        Assert.assertEquals(elements.size(), totalCount / pageSize);
        Mockito.verify(this.consumer).close();
        Mockito.verify(this.producer).close();

        this.consumer.consume();
    }

    @Test(expected = ClosedConsumerException.class)
    public void closeEarly() throws Exception
    {
        final int top = 2;
        List<List<Integer>> elements = new ArrayList<List<Integer>>();

        for (int i = 0; i < top; i++)
        {
            elements.add(this.consumer.consume());
        }

        this.consumer.close();
        Assert.assertEquals(top, elements.size());
        Assert.assertTrue(this.consumer.isConsumed());
        this.consumer.consume();
    }

    @Test
    public void totalAvailable()
    {
        Assert.assertEquals(this.consumer.totalAvailable(), totalCount);
    }

    @Test
    public void doubleClose() throws MuleException
    {
        this.consumer.close();
        this.consumer.close();
    }

    private class TestProducer implements Producer<List<Integer>>
    {

        private int index = 0;

        @Override
        public void close() throws MuleException
        {
        }

        @Override
        public List<List<Integer>> produce()
        {
            List<List<Integer>> ret = new ArrayList<List<Integer>>();

            if (this.index < pages.size())
            {
                ret.add(pages.get(this.index));
                this.index++;
            }

            return ret;
        }

        public int totalAvailable()
        {
            return totalCount;
        };
    }
}
