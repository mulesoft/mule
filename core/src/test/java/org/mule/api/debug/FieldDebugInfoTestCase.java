/*
 * Copyright (c) MuleSoft, Inc.  All rights reserved.  http://www.mulesoft.com
 * The software in this package is published under the terms of the CPAL v1.0
 * license, a copy of which has been included with this distribution in the
 * LICENSE.txt file.
 */

package org.mule.api.debug;

import org.mule.tck.junit4.AbstractMuleTestCase;
import org.mule.tck.size.SmallTest;

import org.junit.Test;

@SmallTest
public class FieldDebugInfoTestCase extends AbstractMuleTestCase
{

    public static final String FIELD_VALUE = "foo";

    @Test(expected = IllegalArgumentException.class)
    public void validatesNullFieldName() throws Exception
    {
        new FieldDebugInfo(null, String.class, FIELD_VALUE);
    }

    @Test(expected = IllegalArgumentException.class)
    public void validatesEmptyFieldName() throws Exception
    {
        new FieldDebugInfo("", String.class, FIELD_VALUE);
    }
}