 ...
 *
 * This is an adaptation of the javax.servlet.Part interface introduced into the Servlet 3.0 specification. The API has been kept
 * that same but the package has changed not to conflict with the Servlet spec.
 *
 * @since 3.0
 */
public interface Part {
   ...
   *
   * <p>
   * This method is not guaranteed to succeed if called more than once for the same part. This allows a particular implementation
   * to use, for example, file renaming, where possible, rather than copying all of the underlying data, thus gaining a
   * significant performance benefit.
   *
   * @param fileName the name of the file to which the stream will be written. The file is created relative to the location as
